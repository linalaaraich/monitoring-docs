...
| R-5 | `SESSION_HANDOFF.md` has no off-host backup | Fold into the planned O-10 nightly S3 backup work |
...
- Full-sweep audit shipped (private `monitoring-audit-results` repo, `audits/2026-04-28-sweep.md`) — system in healthy steady state, all 18 Grafana rules `health=ok`, tests 89/89 (now 95/95 after the philosophy work).
...
- **Pod-level alert rules added (PM)** — `PodHighMemoryUsage` and `PodHighCpuUsage` measure per-pod cgroup utilization (`container_memory_working_set_bytes / container_spec_memory_limit_bytes`) so chaos-induced pod-scoped failures actually trigger alerts. The host-level rules above only fire when an OS-level metric crosses threshold; a single pod hitting its cgroup limit doesn't move host averages on a multi-core node. New rules in a `kubernetes_alerts` group in `roles/grafana/templates/alertrules.yml.j2`.
