...
the project period; Sprint 4 is currently in progress (ten stories
...
\begin{enumerate}
  ...
  \item Bridge each telemetry source (Prometheus, Loki, Jaeger,
        Drain3 templates, triage history) behind an MCP server, and
...
currently in progress (ten stories shipped on day~1, three
...
\begin{itemize}
  ...
  \item \texttt{drain3-mcp:8094}  --- per-service Drain3 template
        miners and novelty-rate queries.
  \item \texttt{rca-history-mcp:8095}  --- read access to the
        persistent RCA history (SQLite, with quality-rated
        \texttt{get\_similar\_decisions} tools).
\end{itemize}
...
The engagement was run as a sequence of two-week scrum sprints tracked
on a Jira board (Figure~\ref{fig:agile}), with every epic and story
followed from discovery through to delivery and audit.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{images/agile-board.png}
\caption{The project board --- epics, stories, and sprint status
tracked throughout the engagement (work-item summary view).}
\label{fig:agile}
\end{figure}
...
\begin{enumerate}
  ...
  \item \textbf{NFR-4 Cost.}  Total monthly run-rate target: under
        ...
        This budget initially drove the 2026-05-19 decision to drop
        the Sprint-4 GPU migration; the decision was reversed on
        2026-05-21 once the cost frame was rebuilt around a Lambda
        autoshutoff gateway (\S\ref{sec:gpu-shipped}).
...
FastAPI service, qwen2.5:14b on Ollama, dashboard, SMTP escalation \\
\hline
\textbf{Layer 2 --- MCP bridge} \\
\texttt{prometheus-mcp}, \texttt{loki-mcp}, \texttt{jaeger-mcp},
\texttt{drain3-mcp}, \texttt{rca-history-mcp} \\
\hline
\textbf{Layer 1 --- Observability foundation} \\
Prometheus, Loki, Jaeger, Grafana, OTel Collector, node-exporter,
cAdvisor, kube-state-metrics, Drain3 poller \\
\hline
\textbf{Layer 0 --- Telemetry sources} \\
Spring Boot (OTel agent), Kong (OTel plug-in), RDS MySQL, k3s pods
and kubelet, host metrics \\
\hline
\end{tabular}
...
\begin{enumerate}
  ...
  \item \textbf{Application host} (\texttt{observability-rca-k3s} on
        ...
        application workload (Spring Boot, Kong, front-end) fronted by
        Kong on a single-node k3s cluster, backed by an external RDS
        MySQL instance.  All three observability signals are sourced
        from Kubernetes: a node-exporter DaemonSet plus
        kube-state-metrics and the kubelet/cAdvisor endpoints supply
        metrics, and an OTel Collector DaemonSet ships pod logs and
        traces off the cluster (see \S\ref{sec:k3s-migration}).
  \item \textbf{AI host} (\texttt{observability-gpu-uswest2},
        AWS EC2 \texttt{g5.xlarge} in \texttt{us-west-2}, NVIDIA
        A10G 24\,GB).  Runs the triage service, the five MCP
        servers, and Ollama with \texttt{qwen2.5:14b-instruct}
        loaded as primary.  The host is reachable over a Tailscale
        tailnet (operator paths) and via an IP-allowlisted API
        Gateway (Grafana webhook ingress); a Lambda autoshutoff
        gateway stops the instance after 20\,min idle (decisions
        D24/D25).  The engineer's laptop
        (WSL2 + Docker Desktop, NVIDIA GTX~1060 6\,GB, MagicDNS
        \texttt{adolin-wsl}) is retained as failover with
...
6  & LLM call (structured output, qwen2.5:14b)     & \texttt{llm\_client.py} \\
...
\begin{itemize}
  ...
  \item \textbf{Local LLM.}  \texttt{qwen2.5:7b-instruct} via Ollama,
        ...
        moved to the laptop's GTX 1060 on 2026-04-23 as the
        Sprint-2-era production runner (later superseded by the
        \texttt{g5.xlarge} GPU host on 2026-05-21, see
        \S\ref{sec:gpu-shipped}).
\end{itemize}

\section{Epic 3 --- Kubernetes migration}
\label{sec:k3s-migration}
...
Spring Boot back-end, Kong, and the front-end; Kong fronts the
employees-backend application as the cluster's API gateway, and the
back-end persists to an external RDS MySQL instance.  The
\texttt{xanmanning.k3s} Ansible Galaxy role provisions k3s as a
\texttt{systemd} unit.

The migration unified all three observability signals onto a
Kubernetes-native collection model.  \textbf{Metrics:} Prometheus
scrapes the kubelet and its embedded cAdvisor for node, pod, and
container resource metrics, the application pods' own
\texttt{/metrics} endpoints, and \textbf{kube-state-metrics} for
cluster object state (pod phase, deployment replica counts, restarts).
A node-exporter DaemonSet still supplies host-level metrics from the
k3s side.  \textbf{Logs:} an OpenTelemetry Collector DaemonSet tails
pod logs off every node and pushes them to Loki, carrying the trace
context so a log line can be correlated to its originating span.
\textbf{Traces:} the same Collector receives OTLP spans from the
Spring Boot OTel agent and the Kong OTel plug-in and exports them to
Jaeger.

Kube-state-metrics also closed a self-monitoring gap.  A single
generic workload-down alert rule (firing when any tracked deployment's
ready-replica count drops below its desired count) routes through the
same Grafana \texttt{triage-webhook} contact point into the AI triage
pipeline, so a crashed application pod is diagnosed by the same path as
any other alert rather than requiring a bespoke probe per service.
...
whitelist (\texttt{prometheus.query}, \texttt{loki.query\_range},
\texttt{jaeger.get\_traces}, \texttt{rca\_history.similar},
\texttt{rca\_history.list\_exemplars}, and
\texttt{rca\_history.get\_exemplar}).  The
...
steady-state latencies on the laptop runner were 25--35\,s warm in
the investigation-window evaluation.  Five fires in that window
were anomalously slow (323, 399, 533, 755, 1081\,s) and the
per-stage instrumentation drafted during the investigation window
held until S3-HF-05 shipped so the retry path could be properly
attributed.  Post-migration measurements on the
\texttt{g5.xlarge} GPU host show median end-to-end latency at
$\sim$38\,s --- the order-of-magnitude improvement that motivated
D24/D25 (\S\ref{sec:gpu-shipped}).
...
\section{The operator-facing platform}
\label{sec:operator-ui}

All of the machinery described in this dissertation surfaces to the
on-call engineer through a single self-hosted dashboard, served by the
triage service itself with no additional component.  This section shows
the surfaces an operator actually uses.

The triage feed (Figure~\ref{fig:ui-feed}) is the landing page: every
decision the platform has made, newest first, with its environment,
service, alert, and verdict.  Cheap-path rows --- deduplicated or
absorbed by a gate before any LLM cost --- sit alongside fully
investigated ones, so the noise the operator never had to see remains
auditable rather than discarded.

\begin{figure}[htbp]
\centering
\includegraphics[height=0.8\textheight]{images/dashboard-feed.png}
\caption{The triage feed --- one row per decision, newest first, with
environment, service, alert and verdict.  The left rail is the
operator's navigation; the top bar carries live counters.}
\label{fig:ui-feed}
\end{figure}

Selecting a row opens the RCA detail (Figure~\ref{fig:ui-rca}): the
named cause in plain language, the model's calibrated confidence, the
cited evidence and suggested \emph{read-only} actions, and the full
fire history of the incident.  The example shown is a real co-fired
incident --- the banner records that its notification was consolidated
with a sibling alert, so the operator received a single page naming
both rather than two competing ones.

\begin{figure}[htbp]
\centering
\includegraphics[height=0.8\textheight]{images/rca-detail.png}
\caption{Anatomy of an RCA: named cause, calibrated confidence, cited
evidence, suggested read-only actions, and the incident's fire history.
This incident was co-fire-consolidated into a single notification.}
\label{fig:ui-rca}
\end{figure}

The Evaluation surface (Figure~\ref{fig:ui-eval}) carries the live
platform-health KPIs above seven-day trends; the Incidents surface
(Figure~\ref{fig:ui-incidents}) groups repeated alerts into one row per
ongoing issue, with a toggle to a per-service rollup.  Both replaced a
set of placeholder pages once the underlying data model was in place.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{images/evaluation-page.png}
\caption{The Evaluation surface --- live health KPIs (top) over
seven-day trends (below), merged onto one page.}
\label{fig:ui-eval}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{images/incidents-page.png}
\caption{The Incidents surface --- one row per ongoing issue, with an
Incidents\,/\,Services segmented toggle.}
...
twenty-five substantive design or rule-of-thumb decisions taken
...
\caption{Catalogue of durable design decisions D1--D25.}
\label{tab:decisions} \\
\toprule
...
Sprint~4 runs 2026-05-20 to 2026-06-03.  Mon 2026-05-25 is the
operational day-1 of the sprint --- the day this chapter was
written --- when ten stories shipped before close, two remain
scheduled across the rest of the plan, and one (SF-5) sits as a
stretch goal in the second week.  The
...
\section{Shipped before day~1 close (ten stories)}

The ten stories below are shipped on the triage service repository
...
\subsection{DA-2 --- Unsafe-action stripping (shipped 2026-05-25, commit 909e40b)}

A clamp-independent gate strips \texttt{kubectl} / \texttt{ssh} /
\texttt{restart} / \texttt{systemctl} / \texttt{reboot} /
\texttt{docker restart} / \texttt{service-name restart} actions
when \texttt{rca\_quality=actionable} \textsc{and} no named cause
keyword (slow query, gc pause, pool exhaustion, deploy, because,
due-to, \ldots) is present in the prose, regardless of LLM
confidence.  Partial-strip semantics on mixed action lists keep
the safe items.  The motivating case is the
\texttt{systemctl restart k3s-node.service} suggestion recorded
in the \S\ref{sec:critical-rca-audit} audit row, which DA-2 now
rewrites to \texttt{action\_taken=shelved} with the unsafe verb
recorded under a new \texttt{unsafe\_actions\_stripped\_total}
Prometheus counter labelled by action-kind.  Seventeen new tests
exercise each verb shape, the cause-keyword recognition, partial
strips, and the audit-canonical case end-to-end.  Test suite
stepped 296 $\to$ 313.

\subsection{Phase 3.A.KPI --- KPI dashboard route (shipped 2026-05-25, commit adbb720)}

A new \texttt{/dashboard/v2/kpi} route surfaces the
\texttt{triage\_precision} / \texttt{triage\_recall} /
suppression-rate / mean-RCA-latency metrics in Grafana-style cards
above the unique-fingerprint table.  Supervisor-requested in the
2026-05-22 design-first review.  Live smoke on
\texttt{observability-gpu-uswest2} at the close of day~1 records
emails-per-day at 1 (7d average 8.4), false-positive rate
0.0\,\% over the one rated decision, median end-to-end latency
25\,s (p95 42\,s over nineteen runs), cheap-path share 65\,\%
(37 cheap vs 20 LLM), and six distinct alertname archetypes in the
last seven days led by \texttt{MediumCpuUsage} at 351 fires.  A
\texttt{KPI / Evaluation} item is added to the dashboard sidebar.
Twenty new tests cover the route, the metric arithmetic, and the
cards' render path.  Test suite stepped 313 $\to$ 333.

\subsection{Day-1 shipping observation}

The ten stories above were originally ranked across two weeks
...
The three items scheduled between day~2 and the sprint close are
listed below in calendar order.  Each is sized in story points;
the sprint-plan source of record is
\texttt{monitoring-docs/sprint4-status.html} \S14.

\begin{itemize}
  \item \textbf{DA-3 cross-row verdict coherence ($\sim$5\,SP).}
        When the same fingerprint fires within $N$ minutes of a
        prior decision, the prior \texttt{cause} is prepended
        into the LLM context with an explicit ``changed my mind
        because\ldots'' framing rule.  Prevents the platform
        from emitting contradictory RCAs across consecutive
        fires of the same flap.
  \item \textbf{URL filter persistence + range filter wire-up
        ($\sim$3\,SP).}  Dashboard filters (verdict, severity,
        alertname-family, time range) are encoded in the URL
        query string so the operator can bookmark and share
        filtered views, and so the back button restores prior
        context.  Range filter wire-up completes the day-1
        schema layer left at half-mast.
  \item \textbf{SF-5 sustained-vs-spike verdict modifier (stretch,
        $\sim$3\,SP).}  Adds a ``duration-above-threshold''
        feature to the alert payload and classifies short-duration
        breaches as \texttt{rca\_quality=transient\_spike} with
        \texttt{action\_taken=shelved}.  Stretch goal --- ships
        only if DA-3 lands clean and the operator-feedback volume
        from SF-7 supports the threshold tuning.
\end{itemize}

The three items together total approximately eleven story
points across the remaining sprint window plus a stretch.  At
Sprint~4 close on 2026-06-03 the suite is expected at
$\sim$350 tests green; the next critical RCA audit (an
eight-criterion repeat of the 2026-05-21 audit recorded in
...
through~3 are closed; Sprint~4 is in active progress with ten
stories shipped on day~1 and three more scheduled across the
...
window 04-30 -- 05-19. \\
4 & 2026-05-20 -- 2026-06-03 & \emph{In progress.}  Day-1
(Mon 2026-05-25) ship of GPU migration (D24/D25, instance live
since 2026-05-21), SF-6 email overhaul, SF-7 operator
feedback page, SF-4 dashboard collapsing, Phase~2.1 detail page,
DA-1/DA-2/DA-4/DA-5 dedupe + unsafe-action stripping,
Phase~3.A.KPI dashboard route; queued: DA-3, URL filters, SF-5
stretch.  Suite \texttt{333/333} green. \\
