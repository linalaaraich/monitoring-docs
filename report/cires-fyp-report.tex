...
Service is responsible for the operational health of the internal
service catalogue that supports port logistics, customer integrations,
and back-office workflows.  At the start of this project, the service
catalogue ran on a small fleet of on-premises virtual machines hosting
a representative web stack (a Spring Boot back-end, a Kong API gateway,
a MySQL data store, a React-based front-end); a parallel migration to
AWS was already on the medium-term roadmap.
...
translation into Tanger Med's context: a small team without a formal
on-call rotation, a mixed-deployment service catalogue, and operational
visibility concentrated in a small number of senior engineers.
...
\begin{enumerate}
  ...
  \item \textbf{Epic 3 --- Drain3 baseline lifecycle (6 SP).}  Wire
        ...
        writing.
  \item \textbf{Epic 4 --- Hallucination firewall + trace depth (15 SP).}
        Added on 2026-04-29 after the \texttt{0b215ef3} incident.
        Two themes: (a) close the three MCP-only-invariant bypasses
        on the LLM-gathering path; (b) wire the
        \texttt{get\_trace(trace\_id)} MCP tool that exists but is
        never invoked.  Five SP shipped same-day as the incident;
        ten SP remaining.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{S3-HF-04 (1 SP, in flight).}  Route
  ...
  \item \textbf{S3-HF-05 (1 SP, blocked on S3-HF-06).}  Route the
        \texttt{bounded\_agency.py} RCA-history paths through
        \texttt{rca-history-mcp} instead of direct \texttt{aiosqlite}.
  \item \textbf{S3-HF-06 (2 SP).}  Extend \texttt{rca\_history\_mcp}
        with quality-rated tools:
        \texttt{get\_similar\_decisions(min\_quality)} and
        \texttt{get\_low\_rated\_examples\_for\_alert}.  Combined
        with S3-FB-01 in one PR.
  \item \textbf{S3-HF-07 (Tier 1, 3 SP).}  Deep trace gather via
        \texttt{get\_trace(trace\_id)} MCP.  The pipeline currently
        gathers Jaeger data at the service-boundary level only and
        can identify ``upstream is slow'' but cannot name what is
        slow.  With trace depth, the LLM sees per-span attributes
        (\texttt{db.statement}, \texttt{http.target}, error tags) for
        the failing trace.  Pre-step (executed during the investigation
        window): sample 12 live Spring Boot traces to confirm JDBC
        parameterisation.  Result: \texttt{db.statement} spans use
        bind parameters (\texttt{?,\,?,\,?}), so PII redaction is
        not a blocker.
  \item \textbf{S3-HF-08 (2 SP, blocked on S3-HF-04 and S3-HF-05).}
  ...
  \item \textbf{S3-HF-09 (Tier 4, 2 SP).}  Chaos scorer 5th axis,
        labelled corpus seed (the \texttt{0b215ef3} incident itself),
        and a CI regression gate.  Must land before Sprint~4's
        Tier~3 iterative agentic gather, which depends on the Tier~4
...
\texttt{exemplars.find\_for\_alert}.  Sprint-4 candidate \#3 stays
...
\begin{itemize}
  \item \textbf{Tier 0} --- strip suggested actions when the confidence
        ...
        Shipped same day as S3-HF-01.
  \item \textbf{Tier 1} --- deep trace gather via
        \texttt{get\_trace(trace\_id)}.  Scoped as S3-HF-07; in flight.
  ...
  \item \textbf{Tier 3} --- iterative agentic gather with
        hypothesis-tree state.  Deferred to Sprint~4; depends on
        Tier~4 being in place first.
  \item \textbf{Tier 4} --- chaos scorer 5th axis, corpus seed,
        regression gate.  Scoped as S3-HF-09.
\end{itemize}
...
accumulated.  The live audit (cron on \texttt{claude-controller})
last ran on 2026-04-29; the cron drift is filed as resilience item
R-1 (chapter~\ref{ch:future}).
...
across a twenty-day low-traffic window is itself an evaluation
result: no incidents observed; AWS instances stayed up;
\texttt{ai-triage-service} on the laptop required exactly one manual
restart over the period (caused by Docker restart-backoff after a WSL
sleep, not by a platform fault).  This validates the architectural
choice of running the AI layer separately from the production
infrastructure: a transient laptop fault does not affect the
observability stack, only the AI-assisted RCAs.
...
form of agentic gather: an unbounded loop (Sprint-4 candidate \#2,
...
The ranked list of Sprint~4 candidates, with the dropped GPU
migration removed, is reproduced in table~\ref{tab:sprint4}.

\begin{longtable}{rp{0.42\textwidth} p{0.13\textwidth} p{0.30\textwidth}}
\caption{Sprint 4 candidates as of 2026-05-19.}
\label{tab:sprint4} \\
\toprule
\textbf{\#} & \textbf{Item} & \textbf{Effort} & \textbf{Gates} \\
\midrule
\endfirsthead
\toprule
\textbf{\#} & \textbf{Item} & \textbf{Effort} & \textbf{Gates} \\
...
\bottomrule
\endfoot
1 & US-5.2 Incident correlator --- 4-alert kill-chain RCA collapsed
   into one coherent email                 & 1--2 d & None \\
2 & Tier 3 --- Iterative agentic gather    & 3--5 d & S3-HF-09 \\
3 & Curated RAG retrieval (BM25 over feedback-curated YAML)
                                           & --- & Epic 2 feedback
                                                 table populated \\
4 & Weekly curation job (APScheduler, Sundays 02:00 UTC)
                                           & --- & Feedback volume \\
5 & Pod-level alert rule fixes (memory cgroup-churn + CPU label
   mismatch)                               & $\sim$30 min each & None \\
6 & Sentence-transformers retriever fallback & --- & BM25 underperforms
                                                    in A/B \\
7 & O-9 webhook auth + O-10 nightly RCA-history S3 backup
                                           & $\sim$30 min each & None \\
8 & MCP native tool-calling rewrite        & 5 SP & --- (no longer
                                                    gated on GPU) \\
9 & Trace-ID linkage from log anomalies    & 2 SP & None \\
10 & RCA playbook templates                & --- & Complements Epic 2 \\
11 & US-5.7 labelled corpus + F1 tracking  & --- & S3-HF-09 \\
12 & L2/L3 chaos scenarios (O-12)          & --- & None \\
13 & Drain3 self-monitoring loop allowlist & $\sim$30 min & None \\
14 & Pipeline duration per-stage instrumentation & 1--2 h & S3-HF-05 \\
15 & Doc debt --- CLAUDE.md refresh, stale PNGs replaced
                                           & 1--2 h & None \\
\end{longtable}
...
runner for twenty-five days with no quality regression surfaced in
...
tool-calling rewrite (Sprint~4 candidate~\#8) was previously gated
