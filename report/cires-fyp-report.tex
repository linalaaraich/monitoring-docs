...
GPU drop (\S\ref{sec:gpu-dropped}) shipped the underlying latency
...
\section{Cloud GPU migration: dropped 2026-05-19, shipped 2026-05-21}
\label{sec:gpu-dropped}

The cloud-GPU migration to \texttt{us-west-2} (\texttt{g5.xlarge}
A10G 24\,GB, host for \texttt{qwen2.5:14b}) sat as a Sprint-4
candidate from 2026-04-27, when the \texttt{G+VT} on-demand vCPU
quota was approved. The decision was \textbf{dropped on cost
grounds on 2026-05-19}, then \textbf{reversed and shipped on
2026-05-21} after a cost-frame rebuild. Both halves of the
decision are recorded here because the reversal is itself the
interesting story.
...
\$730 to \$$\sim$100/mo.  Migration shipped same day as documented
in decisions-log entry D24: instance
\texttt{i-03aec662b1a47ad8f}, Tailscale hostname
\texttt{observability-gpu-uswest2}, \texttt{qwen2.5:14b-instruct}
primary with the laptop runner retained as failover (a
DNS-level flip on Grafana's webhook target).  Phase~2 smoke test
on the new runner produced end-to-end median $\sim$38~s versus
the laptop's $\sim$5~min baseline --- an $\sim$8$\times$ speedup
without behavioural regressions (test suite stays 252/252 green).
A co-shipped access lockdown (D25) routes the Grafana webhook
through an IP-allowlisted API Gateway, restricts operator paths
...
concurrency, MCP native tool-calling rewrite) ships as
defensive margin rather than as the latency fix --- the
migration addressed the underlying single-LLM-on-consumer-GPU
ceiling that the bundle was attempting to work around.
Second, the \texttt{us-west-2} G+VT quota approval (4 vCPU,
granted 2026-04-27) is now in use rather than held in reserve.
Third, the lesson on the methodology: cost-grounds drops should
specify the duty-cycle assumption explicitly; the 2026-05-19
decision was correct under a 24/7 frame and wrong under a
bursty-duty-cycle frame, and the frame was the load-bearing
assumption.
