...
collectors (Prometheus scrape, Promtail, OTel Collector) and into
storage (Prometheus TSDB, Loki, Jaeger).  Grafana surfaces the
data and routes alerts through the unified-alerting webhook
contact point.}
...
\begin{itemize}
  \item \textbf{SF-13 (keystone, $\sim$8\,SP).}  Deploy a
        ...
        Prometheus, logged to Loki via Promtail.  Helm charts
        live in the \texttt{monitoring-project} repository; the
        bed is reproducible from clean k3s via a single
