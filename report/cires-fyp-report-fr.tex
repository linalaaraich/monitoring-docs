...
sprint~4 est actuellement en cours (dix user stories livrées dès son
...
sprint~4 actuellement en cours (dix user stories livrées le
jour~1, trois programmées sur le reste du plan). Le
...
Le projet a été conduit comme une suite de sprints scrum de deux
semaines suivis sur un tableau Jira (Figure~\ref{fig:agile}), chaque
épopée et chaque récit étant suivis de la découverte jusqu'à la
livraison et l'audit.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{images/agile-board.png}
\caption{Le tableau de projet --- épopées, récits et état des sprints
suivis tout au long de la mission (vue de synthèse des éléments de
travail).}
\label{fig:agile}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{LLM local.} \texttt{qwen2.5:7b-instruct} via Ollama,
        ...
        promu à \texttt{qwen2.5:14b-instruct} sur hôte GPU AWS
        \texttt{g5.xlarge} (\S\ref{sec:gpu-shipped}) le 2026-05-21,
        portable conservé en repli.
\end{itemize}
...
La plateforme a maintenu un régime permanent sur la fenêtre
d'investigation de vingt jours --- plus une vague de clôture de
sprint le 2026-05-19 (dix SP de refactorings à travers quatre
dépôts sans régression) --- résultat d'évaluation en soi. Aucun
incident sur les vingt-sept jours du 2026-04-23 (clôture sprint~2)
au 2026-05-20~; instances AWS continues~; le conteneur
\texttt{ai-triage-service} sur le \emph{runner} portable n'a requis
qu'un redémarrage manuel sur la période (Docker
\emph{restart-backoff} après mise en veille WSL, pas une
...
production. Depuis la migration GPU du 2026-05-21
(\S\ref{sec:gpu-shipped}), le conteneur tourne sur
\texttt{observability-gpu-uswest2} avec portable en repli~; le
même invariant tient.

\section{La plateforme côté opérateur}
\label{sec:operator-ui}

Toute la mécanique décrite dans ce mémoire est exposée à l'ingénieur
d'astreinte via un unique tableau de bord auto-hébergé, servi par le
service de triage lui-même, sans composant supplémentaire. Cette
section montre les surfaces qu'un opérateur utilise réellement.

Le flux de triage (Figure~\ref{fig:ui-feed}) est la page d'accueil~:
chaque décision prise par la plateforme, la plus récente en tête, avec
son environnement, son service, son alerte et son verdict. Les lignes
du chemin économique --- dédupliquées ou absorbées par un filtre avant
tout coût LLM --- côtoient les investigations complètes, de sorte que
le bruit que l'opérateur n'a jamais eu à voir reste auditable plutôt
que jeté.

\begin{figure}[htbp]
\centering
\includegraphics[height=0.8\textheight]{images/dashboard-feed.png}
\caption{Le flux de triage --- une ligne par décision, la plus récente
en tête, avec environnement, service, alerte et verdict. Le rail de
gauche est la navigation de l'opérateur~; la barre supérieure porte les
compteurs en direct.}
\label{fig:ui-feed}
\end{figure}

Sélectionner une ligne ouvre le détail de la RCA
(Figure~\ref{fig:ui-rca})~: la cause nommée en langage clair, la
confiance calibrée du modèle, les preuves citées et les actions
suggérées \emph{en lecture seule}, ainsi que l'historique complet des
tirs de l'incident. L'exemple montré est un incident réel
co-déclenché --- le bandeau indique que sa notification a été
consolidée avec une alerte s\oe ur, de sorte que l'opérateur a reçu un
seul message les nommant toutes deux plutôt que deux messages
concurrents.

\begin{figure}[htbp]
\centering
\includegraphics[height=0.8\textheight]{images/rca-detail.png}
\caption{Anatomie d'une RCA~: cause nommée, confiance calibrée, preuves
citées, actions suggérées en lecture seule, et historique des tirs de
l'incident. Cet incident a été consolidé en une seule notification.}
\label{fig:ui-rca}
\end{figure}

La surface Évaluation (Figure~\ref{fig:ui-eval}) porte les indicateurs
de santé en direct au-dessus des tendances sur sept jours~; la surface
Incidents (Figure~\ref{fig:ui-incidents}) regroupe les alertes répétées
en une ligne par problème en cours, avec une bascule vers une synthèse
par service.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{images/evaluation-page.png}
\caption{La surface Évaluation --- indicateurs de santé en direct (en
haut) au-dessus des tendances sur sept jours (en bas), fusionnés sur
une seule page.}
\label{fig:ui-eval}
\end{figure}
...
\caption{La surface Incidents --- une ligne par problème en cours, avec
une bascule segmentée Incidents\,/\,Services.}
...
(2026-05-25) est celui où ce chapitre a été rédigé~; dix user
stories ont été livrées avant la fin de la journée, deux restent
...
Le \emph{bundle} de latence P0 original du sprint~4
(instrumentation par étape, \emph{timeout} 180~s, concurrence
sérielle Ollama, réécriture \emph{native tool-calling} MCP) est
livré comme marge défensive plutôt que comme correction de
latence --- la migration a traité le plafond \og~LLM unique sur
GPU grand public~\fg{} sous-jacent. Le quota G+VT
\texttt{us-west-2} (4 vCPU, octroyé le 2026-04-27) passe en usage
plutôt qu'en réserve. Leçon méthodologique~: les abandons pour
raisons de coût doivent expliciter l'hypothèse de cycle d'usage~;
la décision du 2026-05-19 était correcte sous un cadre 24/7 et
fausse sous un cadre \emph{bursty}, et le cadre était l'hypothèse
porteuse.
...
\subsection{DA-2 --- Retrait des actions non sûres (livré 2026-05-25, commit 909e40b)}
...
\subsection{Phase 3.A.KPI --- Route KPI (livrée 2026-05-25, commit adbb720)}
...
Les dix user stories ci-dessus étaient classées sur les deux
semaines du plan jour par jour et ont été achevées le jour~1. La
clôture du sprint~3 a laissé les surfaces suite, tableau de bord
et notificateur dans un état où de petits \emph{commits} bien
périmétrés atterrissaient à haute vélocité, et la migration GPU a
libéré la marge de latence permettant de tester en secondes plutôt
qu'en minutes. Le travail restant en file d'attente est donc
proportionnellement plus important par élément (DA-3 est à
...
sprint, par ordre calendaire (source de référence~:
...
Boucle d'appel d'outils sur trois itérations, chacune étoffant un
arbre d'hypothèses partiel via un appel MCP, jusqu'à seuil de
confiance, limite de profondeur ou budget de latence épuisé.
Matériellement plus permissif que la reprise à action contenue~;
le conditionnement à S3-HF-09 (désormais satisfait) reflète la
position selon laquelle livrer Tier~3 sans barrière automatisée de
régression de qualité serait imprudent.

\paragraph{Récupération RAG curée (US-3.3 / US-3.4).}
Une fois la table de retour SF-7 suffisamment alimentée, un job
hebdomadaire écrit les entrées notées au-dessus d'un seuil dans
\texttt{library\_curated.yaml}~; la section exemplaire du
\emph{prompt} est complétée par BM25 ($k=3$ positifs, $k=2$
anti-exemples) sur ce corpus. Le \emph{dry-run} de la fenêtre
d'investigation a montré BM25 dégénéré sur la bibliothèque de base
à quatorze archétypes~; le scénario de production est BM25 sur une
bibliothèque enrichie, mesuré une fois SF-7 alimenté.
...
Le sprint~5 (2026-06-06 au 2026-06-17) déplace le centre de gravité
de la plateforme vers l'extérieur. Les sprints~1--4 ont durci la
boucle de tri centrale contre une pile web représentative (Spring
Boot + Kong + MySQL + React, plus \texttt{car-rental} arrivé en
clôture du sprint~3). Le mandat du sprint~5 est double~: déployer
un banc microservice canonique exerçant les chemins
multi-applicatifs et livrer la configuration opérateur et le
durcissement production en file d'attente depuis les sprints~3--4.

Le sprint est structuré en trois épopées concurrentes~; la
capacité à trois ingénieurs signifie un propriétaire dédié et deux
relecteurs par épopée, avec mise en binôme explicite aux points
d'intégration (banc microservice $\times$ Drain3 hiérarchiques,
table d'incidents $\times$ onglet phase 3.B). La
figure~\ref{fig:sprint-roadmap} situe le sprint~5 dans la
chronologie globale.
...
dans la détection d'anomalies. Le \emph{poller} Drain3 actuel
déclenche \texttt{Drain3AnomalyDetected} via un unique seuil par
service sur le taux de modèles nouveaux~: il ne peut distinguer
\og~un service bizarre tout seul~\fg{} de \og~beaucoup de services
discrètement bizarres en même temps~\fg{}. Une dérive corrélée
sous le seuil par service est silencieuse, alors même que le
système global a bougé.
...
Discussion prospective du 2026-05-25 sur la pile par hôte au
passage à la production CIRES. La contrainte NOC --- pas de
nouveaux \emph{exporters} en production, Zabbix reste l'incumbent
métriques --- oriente vers un OTel Collector unique absorbant
logs, traces et métriques via les récepteurs \texttt{hostmetrics},
\texttt{prometheus} et \texttt{kubeletstats}, éliminant
\emph{node-exporter}. Le détail complet (récepteurs, propriétés
d'audit, contrepartie mémoire $\sim$80--150~MB) figure dans la
version anglaise du mémoire et dans
\texttt{solution-brief.html} §13.8.
...
Technologies. Les sprints~0--3 sont clôturés~; le sprint~4 est en
cours actif avec dix user stories livrées le jour~1 et trois
programmées sur le reste du plan. La plateforme répond à l'écart
opérationnel identifié pendant la recherche de terrain du sprint~0
--- l'équipe découvrait les problèmes de production après coup ---
en combinant une pile d'observabilité \emph{open source} classique
avec une couche de tri par IA transformant chaque alerte en une
RCA structurée pré-investiguée.
