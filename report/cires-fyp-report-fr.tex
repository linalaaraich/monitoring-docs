...
tri affichait \texttt{296/296} au vert le premier jour du sprint~4
...
\begin{itemize}
  ...
  \item \textbf{Loki} pour les journaux. Ingestion en mode
        \emph{pull} via Promtail. Loki stocke les flux de journaux
        indexés par étiquettes plutôt que par contenu plein texte,
        ce qui maintient les coûts de stockage faibles et s'aligne
        proprement sur le modèle d'étiquettes de Prometheus.
...
(scrape Prometheus, Promtail, OTel Collector) et le stockage
(Prometheus TSDB, Loki, Jaeger). Grafana expose les données et
achemine les alertes via le point de contact webhook de l'alerting
unifié.}
...
\section{Livré avant la clôture du jour 1 (huit user stories)}

Les huit user stories ci-dessous sont livrées sur le dépôt du
service de tri avec la suite de tests à \texttt{296/296} au vert et
...
Les huit user stories ci-dessus étaient initialement classées sur
...
par élément (la cohérence inter-lignes DA-3 et la route
\texttt{/dashboard/v2/kpi} sont toutes deux à l'échelle de la
journée, pas de l'heure).

\section{En file d'attente pour le reste du sprint 4}

Les cinq éléments programmés entre le jour~2 et la clôture du
sprint sont listés ci-dessous par ordre calendaire. Chacun est
dimensionné en points d'histoire ou en heures selon le cas~; la
source de référence du plan de sprint est
\texttt{monitoring-docs/sprint4-status.html} \S14.

\begin{itemize}
  \item \textbf{Mardi 2026-05-26 --- DA-2 retrait des actions non
        sûres ($\sim$3\,SP).} Une barrière indépendante du
        bridage qui retire les actions \texttt{kubectl} /
        \texttt{ssh} / \texttt{restart} / \texttt{systemctl}
        lorsque \texttt{rca\_quality=actionable} \textsc{et}
        qu'aucune cause nommée n'est présente dans la prose, quelle
        que soit la confiance LLM. Le cas motivant est la
        suggestion \texttt{systemctl restart k3s-node.service}
        consignée dans la ligne d'audit \S7c.4.
  \item \textbf{Mercredi 2026-05-27 --- Route tableau de bord
        phase 3.A.KPI ($\sim$3\,SP).} Une nouvelle route
        \texttt{/dashboard/v2/kpi} faisant émerger les métriques
        \texttt{triage\_precision} / \texttt{triage\_recall} /
        taux de suppression / latence moyenne RCA sous forme de
        cartes de style Grafana au-dessus du tableau d'empreintes
        uniques. Demandé par le superviseur.
  \item \textbf{Jeudi 2026-05-28 --- DA-3 cohérence verdict
        inter-lignes ($\sim$5\,SP).} Lorsque la même empreinte se
        déclenche dans les $N$~minutes d'une décision antérieure,
        la \texttt{cause} antérieure est préfixée dans le contexte
  ...
  \item \textbf{Vendredi 2026-05-29 --- Persistance des filtres
        URL + câblage du filtre de plage ($\sim$3\,SP).} Les
        filtres du tableau de bord (verdict, sévérité, famille
        d'\emph{alertname}, plage temporelle) sont encodés dans la
        chaîne de requête URL afin que l'opérateur puisse marquer
        et partager des vues filtrées, et que le bouton retour
        restaure le contexte antérieur. Le câblage du filtre de
        plage achève la couche de schéma du jour~1 laissée à
        mi-chemin.
  \item \textbf{Vendredi S2 (étirement) --- SF-5 modificateur de
        verdict soutenu vs.\ pic ($\sim$3\,SP).} Ajoute une
        caractéristique \og~durée au-dessus du seuil~\fg{} à la
        charge utile d'alerte et classifie les franchissements de
        courte durée comme \texttt{rca\_quality=transient\_spike}
        avec \texttt{action\_taken=shelved}. Objectif
        d'étirement --- ne sera livré que si DA-3 atterrit
        proprement et que le volume de retour d'opérateur issu de
        SF-7 soutient le réglage du seuil.
\end{itemize}

Les cinq éléments totalisent ensemble environ dix-sept points
d'histoire sur quatre journées de travail plus un étirement. À la
clôture du sprint~4 le 2026-06-05, la suite est attendue à
$\sim$310 tests au vert~; le prochain audit critique RCA (une
...
\begin{itemize}
  \item \textbf{SF-13 (clé de voûte, $\sim$8\,SP).} Déployer un
        ...
        instrumenté OTel, scrapé par Prometheus, journalisé vers
        Loki via Promtail. Les \emph{Helm charts} vivent dans le
        dépôt \texttt{monitoring-project}~; le banc est
        reproductible depuis k3s vierge via un seul lot de
        \texttt{helm install}.
...
L'esquisse originale incluait un quatrième palier --- applications
partageant une base de données, un \emph{namespace} ou un hôte. Il
a été abandonné comme combinatoirement flou~: différentes
définitions d'intersection (db-partagée vs.\ ns-partagé vs.\
hôte-partagé) se chevauchent confusément, et leur exposition
comme regroupements réglables fait exploser la surface de
configuration opérateur pour un signal supplémentaire limité que
Tier~3 couvre déjà comme filet de sécurité. Les trois portées
hiérarchiques propres ci-dessus sont conservées~; l'intersection
est laissée de côté par dessein.
...
\begin{itemize}
  \item \textbf{Un modèle de déploiement multi-tenant.} La
        plateforme actuelle est mono-tenant par construction. Un
        déploiement multi-tenant (par exemple un service de tri par
        unité métier à Tanger Med, avec des ponts MCP partagés et
        une pile d'observabilité partagée) requerrait une
        isolation par \emph{namespace} dans l'historique RCA, des
        bibliothèques d'exemplaires par tenant et un calibrage de
        confiance par tenant. Périmètre naturel du sprint~6+.
  \item \textbf{Intégration directe avec les systèmes de production
        de Tanger Med.} La plateforme est actuellement exercée
        contre une pile web représentative et un banc microservice
        (après sprint~5). L'\emph{onboarding} de services de
        production réels demanderait une gestion soigneuse des
        PII (contenu de retour d'opérateur, lignes de journal), un
        programme SLO pour fixer des seuils de confiance
        significatifs et un processus formel de contrôle des
        changements pour les mises à jour de \emph{prompt} et
        d'exemplaires. C'est le chemin vers la transition
        production avec greenlight du superviseur que le mémo de
        périmètre du projet (\texttt{[CIRES scope --- PoC vs
        production]}) identifie comme l'état terminal du projet.
  \item \textbf{Évaluation fédérée de modèles.} À mesure que le
        projet accumule des issues RCA étiquetées via la couche de
        retour ingérée par SF-7 et SF-13, comparer des choix
        alternatifs de LLM sur un échantillon retenu devient
        faisable. Modèles dignes d'évaluation aux côtés de
        l'incumbent \texttt{qwen2.5:14b-instruct}~:
        \texttt{deepseek-r1:14b-distill},
        \texttt{mistral-nemo:12b}, et le plus grand
        \texttt{qwen2.5:32b-instruct} maintenant que l'hôte GPU a
        de la marge en VRAM. La courbe coût-vs-qualité sera le
        facteur décisif~; c'est le successeur naturel de la
        migration GPU D24.
  \item \textbf{Observabilité prédictive (carve-out EPIC12).}
        L'onglet signaux détectifs S4-PRED-01 absorbé dans
        EPIC14 Phase 3.A.Anomalies est le pied dans la porte~;
        PRED-02 piste de capacité, PRED-03 lignes de base de
        bandes de saturation et PRED-04 prédicteur de cascade
        sont des candidats sprint~6 qui poussent la plateforme du
        diagnostic vers le territoire prédictif.
\end{itemize}
...
\begin{enumerate}
  \item \textbf{Une mise en \oe{}uvre de référence de bout en bout.}
        Une pile d'observabilité provisionnée par Terraform,
        configurée par Ansible, exécutée sur k3s-et-Docker sur AWS,
        avec un service de tri par IA basé sur FastAPI produisant
        des RCA structurées à partir des webhooks Grafana via un
        LLM hébergé localement (\texttt{qwen2.5:14b-instruct} sur
        un hôte GPU AWS \texttt{g5.xlarge} dédié depuis le
        2026-05-21, \texttt{qwen2.5:7b} sur portable conservé en
        repli). Reproductible depuis une infrastructure vierge et
        exploitable depuis une seule commande de \emph{playbook}.
  \item \textbf{L'invariant d'accès aux données via MCP uniquement.}
        Une règle à l'échelle du projet, en partie architecturale
        et en partie mécanique (via le lint S3-HF-08), selon
        laquelle chaque donnée que voit le LLM doit transiter par
        un pont MCP. La règle produit un régime de provenance sur
        l'entrée du LLM et est appliquée par les métriques de la
        couche pont, les rapports d'audit et un lint
        d'intégration continue.
  \item \textbf{Le pare-feu anti-hallucination.} Une combinaison
        en couches de mécanismes (injection d'exemplaires, liste
        noire par alerte, validateur de surface, validateur de
        menu d'hypothèses, règle cause-preuves, bridage de
        confiance avec retrait des actions, reprise à action
        contenue) conçue pour garder la sortie LLM digne de
        confiance sur le plan opérationnel. Chaque mécanisme
        répond à un mode de défaillance spécifique observé
        empiriquement pendant la construction et consigné dans le
        journal des décisions avec les preuves qui l'ont
        motivé.
  \item \textbf{La couche de retour d'information de l'opérateur
        en boucle fermée.} Un schéma et une surface qui capturent
        les corrections et les notations d'opérateur sur les RCA
        (livrés de bout en bout via SF-6 et SF-7 le jour~1 du
        sprint~4) et les propagent dans la couche de suppression,
        dans les métriques de précision/rappel et (au sprint~5)
        dans le corpus de récupération curé.
  \item \textbf{Un journal des décisions documenté.} Vingt-cinq
        décisions durables de conception (D1--D25) consignées de
        manière contemporaine aux travaux d'ingénierie,
        comprenant le problème observé, les contraintes, la
        décision et la vérification. Le journal est un livrable
        primaire du projet et est exerçable lors d'audits
        futurs.
\end{enumerate}

La plateforme est en exploitation active contre une pile web
représentative sur AWS, avec des audits quotidiens statiques et
en condition réelle produisant une trace documentée de santé
opérationnelle. La latence médiane de bout en bout du pipeline
s'établit à $\sim$38\,s sur l'hôte GPU (en baisse depuis
$\sim$5\,min sur le \emph{runner} portable)~; la suite de tests
du service de tri est à \texttt{296/296} au vert le jour~1 du
sprint~4~; le lint d'invariant MCP-seul est propre depuis
l'atterrissage de S3-HF-08 le 2026-05-19~; l'hôte GPU est en
service sans interruption depuis la migration du 2026-05-21. Le
travail du sprint~5 capturé au chapitre~\ref{ch:sprint5} --- un
banc de test microservice canonique, un modèle de données au
niveau incident sur le tableau de bord, des surfaces de
configuration opérateur et la nouvelle conception de détection
d'anomalies Drain3 hiérarchique --- est l'évolution naturelle
suivante et le corps de travail qui déterminera si la plateforme
est prête pour la transition production avec greenlight du
superviseur qui constitue l'état terminal de ce projet.

Le choix méthodologique qui a conditionné la collecte agentique
itérative (Tier~3) à un échafaudage de mesure (Tier~4) est, de
l'avis de l'auteure, la leçon la plus durable que le projet
emporte de sa phase de construction --- le coût d'expédition d'un
mécanisme IA non mesurable en production excède matériellement le
coût de construction préalable de la mesure. La même discipline
informe la conception S5-DRN-01 des seuils Drain3 hiérarchiques
du sprint~5~: l'intuition architecturale est arrivée le
2026-05-25, mais la conception est explicitement conditionnée au
fait que le banc microservice (SF-13) soit en service afin que le
scénario d'acceptation inter-applications puisse être exercé sur
des données réelles avant que la logique de seuils multi-paliers
ne soit autorisée à approcher le trafic de production.
...
détail phase~2.1, déduplication DA-4/DA-5~; en file d'attente~:
DA-2, phase~3.A.KPI, DA-3, filtres URL, SF-5 en étirement. Suite
\texttt{296/296} au vert. \\
